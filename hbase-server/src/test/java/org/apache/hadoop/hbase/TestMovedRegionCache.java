...
public class TestMovedRegionCache {
  ...
  public void setup() throws Exception {
    ...
    Assert.assertEquals(2, cluster.getRegionServerThreads().size());
  ...
  public void testMovedRegionsCache() throws IOException, InterruptedException {
    ...
    Assert.assertNotNull("Moved region NOT in the cache!", source.getMovedRegion(
      movedRegionInfo.getEncodedName()));
    Thread.sleep(source.movedRegionCacheExpiredTime());
    Assert.assertNull("Expired moved region exist in the cache!", source.getMovedRegion(
      movedRegionInfo.getEncodedName()));
  }
}
